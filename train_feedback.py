"""
TRAINING SETUP FOR FEEDBACK TRANSFORMER
"""
import time
...
trainer_flags = {
    "default_root_dir": "./cogs_benchmark_feedback",
    "amp_backend": "native",
    "benchmark": False,
    "deterministic": False,
    "callbacks": [
        ModelCheckpoint(monitor="epoch_val_accuracy", dirpath="./cogs_benchmark_feedback"),
        EarlyStopping(monitor="epoch_val_accuracy", mode="max", patience=2),
    ],
    "gpus": 1,
    "log_every_n_steps": 10,
    "logger": TensorBoardLogger(save_dir="logs/", name="cogs_benchmark_feedback"),
    "max_epochs": 100,
...
feedback_trainer = pl.Trainer(**trainer_flags)
feedback_trainer.fit(model=feedback_model, datamodule=datamodule)


"""
TEST SEQ2SEQ FEEDBACK TRANSFORMER FOR COGS BENCHMARK GENERALIZATION TEST SET
"""
torch.cuda.empty_cache()

start_time = time.time()
feedback_trainer.test()
